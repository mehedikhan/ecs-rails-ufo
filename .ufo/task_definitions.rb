...
task_definition "ecs-rails-ufo-worker" do
  source "fargate" # will use ufo/templates/fargate.json.erb
  variables(
    family: task_definition_name,
    name: "worker",
    # Comment out awslogs_* if you do not want logs to be sent to CloudWatch.
    awslogs_group: ["ecs/ecs-rails-ufo-worker", Ufo.env_extra].compact.join('-'),
    awslogs_stream_prefix: "ecs-rails-ufo",
    awslogs_region: helper.current_region,
...
task_definition "ecs-rails-ufo-clock" do
  source "fargate" # will use ufo/templates/fargate.json.erb
  variables(
    family: task_definition_name,
    name: "clock",
    # Comment out awslogs_* if you do not want logs to be sent to CloudWatch.
    awslogs_group: ["ecs/ecs-rails-ufo-clock", Ufo.env_extra].compact.join('-'),
    awslogs_stream_prefix: "ecs-rails-ufo",
    awslogs_region: helper.current_region,
